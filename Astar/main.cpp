...
struct Kratka {
    ...
    void add_neighbors(const vector<vector<Kratka>>& grid) {
        if (x - 1 >= 0 && !grid[x - 1][y].obstacle) // up
        {
            neighbors.push_back(grid[x - 1][y]);
        }

        if (x + 1 < 20 && !grid[x + 1][y].obstacle) // down
        {
            neighbors.push_back(grid[x + 1][y]);
        }

        if (y - 1 >= 0 && !grid[x][y - 1].obstacle) // left
        {
            neighbors.push_back(grid[x][y - 1]);
        }

        if (y + 1 < 20 && !grid[x][y + 1].obstacle) // right
        {
            neighbors.push_back(grid[x][y + 1]);
        }
    }
...
double calculate_h(const Kratka& next, const Kratka& cel) {
    return sqrt(pow(cel.x - next.x, 2) + pow(cel.y - next.y, 2));
}

void show_grid(const vector<vector<int>>& grid) {
    for (int i = 0; i < 20; i++) {
        for (int j = 0; j < 20; j++) {
            cout << grid[i][j] << " ";
        }
...
void display_pos(const Kratka& kratka) {
    cout << "x:" << kratka.x << " " << "y:" << kratka.y << " " << "g:" << kratka.g << " " << "h:" << kratka.h << " " << "f:" << kratka.f << endl;
}

vector<vector<int>> load_grid(const string& file_name) {
...
vector<vector<Kratka>> kratka_grid(const vector<vector<int>>& grid) {
...
bool check(const vector<Kratka>& vect, const Kratka& kratka) {
    bool temp = false;
...
int main() {
    ...
    Kratka start = grid_kratka[0][0];
    Kratka cel = grid_kratka[19][19];

    if (start.obstacle) {
        cout << "There's an obstacle at start, no path has been found" << endl;
        return 0;
    }
